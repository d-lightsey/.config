...
3. **Display Result**
   ...
   - Run /plan {N} to create implementation plan
   - Run /implement {N} to generate final cost analysis PDF
   ```
...
**Note**: Final cost analysis PDF (`strategy/cost-analysis-*.pdf`) is generated by `/implement`, not `/sheet`.
...
/implement {N}          -> Executes plan, generates strategy/cost-analysis-*.pdf
```
